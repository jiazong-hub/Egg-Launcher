# Egg Launcher 0.9.1 用户功能说明

本文说明 0.9.1 公测版的页面、操作和能力边界。启动器是 llama.cpp 与 ChatGPT Desktop 之间的管理外壳，不重复实现推理、KV Cache、模型调度或语义摘要。
...
窗口按顺序使用五个选项卡：`基础参数`、`视觉模块`、`MTP 推测解码`、`高级参数`、`模型信息`。每个选项卡独立滚动，底部恢复、取消、保存与保存为默认按钮保持固定。
...
- 产品：Egg Launcher；版本：0.9.1 公测版；开发者及版权主体：甲总不是贾总。
