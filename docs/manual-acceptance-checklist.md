...
## 0.9.3 公开测试版实测记录（2026-09-26）

- 测试环境：Windows 10 企业版 22H2，操作系统内部版本 19045.5371。
- 启动、基本使用、界面动画、模式切换与启动稳定；全新安装和旧版升级顺利完成。
- NVIDIA CUDA 后端当前实测稳定；AMD Vulkan 后端可能偶发重连报错；ROCm 后端未测试。
- 建议通过 Egg Launcher 执行模式切换并启动客户端；绕过启动器可能造成客户端接口状态不同步。
- 卸载流程和卸载时恢复 OpenAI 配置尚未测试。以下安装专项条目保留用于补测尚未覆盖的环境与边界。
...
1. 已完成一次全新安装；补测未安装 .NET 的干净 Windows 10 22H2 x64 与 Windows 11 x64 标准用户环境，确认无需管理员提权、快捷方式与卸载项正确，首次安装不自动注册登录启动。
2. 已完成一次旧版升级；补充记录升级来源版本，并验证 0.9.2 便携版迁移时设置、模型路径和旧 Local 任务仍可用，旧便携目录不被删除。用户原已启用登录启动时，检查安装版启动路径是否修复到安装目录，并验证目录 ACL 通过启动项安全校验。
