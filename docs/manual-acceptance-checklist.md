...
- 本地 v0.9.2 标签封存时 Release 构建通过，自动化测试和下列完整沙箱手工验收尚未执行。2026-09-25 的后续工作树已有 249 项自动化测试和 Release 构建通过记录；下列真机验收仍未执行。
...
3. ChatGPT Desktop 启动后验证：
   - 当前官方模型仍为官方 Provider；本地模型不作为可选在线模型出现；配置中只保留未激活、指向 `127.0.0.1:0` 的本地历史 Provider 定义，活动 Local 端点已撤销；
   - 原 OpenAI 账户仍保持登录；
   - 在线模型与官方工具恢复正常；
...
## 0.9.3 EXE 安装包专项验收（尚未执行）

1. 在未安装 .NET 的干净 Windows 10 22H2 x64 与 Windows 11 x64 标准用户环境中安装；确认无需管理员提权，快捷方式与卸载项正确，首次安装不自动注册登录启动。
2. 从 0.9.2 便携版直接安装 0.9.3；确认设置、模型路径和旧 Local 任务仍可用，旧便携目录不被删除。用户原已启用登录启动时，首次打开安装版后检查启动路径是否修复到安装目录，并验证目录 ACL 通过启动项安全校验。
3. 保持启动器或 Agent 运行时尝试安装、覆盖升级或卸载；确认操作停止，进程不被强制结束，旧安装与用户数据可继续使用。
4. 在 OpenAI 模式卸载；确认只移除安装文件、快捷方式和指向安装版的登录启动项，不修改其他路径的启动项，不删除 `%LOCALAPPDATA%\ChatGPTLocalLauncher`、模型、Runtime、ChatGPT Desktop 或旧任务。
5. 在 Local 模式且 Desktop 已关闭时卸载；确认卸载前恢复官方模型与 Provider，保留 `127.0.0.1:0` 离线历史入口，并能查阅旧 Local 任务。模拟恢复记录缺失或配置冲突时，确认卸载停止且程序文件仍在。
6. 核验 EXE 的 SHA-256、安装包及卸载程序签名、安装后的 `BUILD-INFO.txt` 与 `SHA256SUMS.txt`；安装目录内容应与内部发布目录一致。
