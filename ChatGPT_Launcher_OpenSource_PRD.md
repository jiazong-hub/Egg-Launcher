...
## 0.2 v0.9.1 实现补充（2026-09-21）

- 软件对外名称为 Egg Launcher，核心程序集仍沿用 ChatGPT Local Launcher 命名；该差异是产品展示名与内部项目名的区分，不影响配置兼容性。
- 主界面采用首页、模式、模型三段导航，支持深色/浅色主题、中文/英文及跟随 Windows 显示语言、托盘按需状态和轻量统一动画。
- 模型在首次添加时识别 Dense/MoE，结果写入 Profile 后不再每次启动扫描；未知类型必须由用户指定，类型变更重建参数配置。
- 参数编辑按基础、视觉、MTP、高级、模型信息五个选项卡组织；未调整的参数不写入配置，继续由 llama.cpp 决定默认值。
- 思考档位、MTP 和视觉能力都遵循“明确能力才可启用”的原则。检测或隔离验证位于模型管理，参数页只负责启用与运行参数。
- 主模型下载完成后可以自动登记；MTP/mmproj 只下载不关联。Hugging Face 缓存目录、无扩展名 blob、snapshot 和多分片 GGUF 均由扫描解析层处理。
- 从模型列表移除只清理 Launcher 配置和受管生成物，不删除模型数据；缓存删除继续使用独立、受保护的 llama 原生流程。
- 状态监控仅在前台低频运行，后台暂停；模型管理、托盘菜单和悬停状态按用户动作刷新，避免无意义常驻消耗。
